estimate_pooled_did_iv <- function(
    data,
    id_var,
    outcome_var,
    cum_out_var,
  ...
  required_vars <- c(id_var, outcome_var, cum_out_var, time_var, 
                     treated_var, date_treated_var, control_vars, iv)
  ...
  process_cohort <- function(treat_date, dt_full,
                             ...
                             control_vars, estimand, balancing_method, iv) {
    
   # dt_full<-dt
   # treat_date<-as.IDate(19631)
    ...
    keep_vars <- c(id_var, control_vars, time_var, cum_out_var, 
                   outcome_var, date_treated_var, treated_var, iv)
    ...
    tryCatch({
      ...
      did_iv_formula <- as.formula(paste(outcome_var, "~ after+", treated_var, "| did~", paste(iv, collapse = "+")))
      did_coef <- coef(feols(did_iv_formula, data = cohort_dt))[2]
