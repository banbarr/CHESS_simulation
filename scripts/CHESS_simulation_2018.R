...
study_pop[, pr:=
            exp(0.005*age_2018+0.005*imd+0.005*cvd+0.005*dep+random_slope*c_time-random_slope_sq*time_sq+intercept)
                /(1+
            exp(0.005*age_2018+0.005*imd+0.005*cvd+0.005*dep+random_slope*c_time-random_slope_sq*time_sq+intercept)
                  )]
...
study_pop_ag<-study_pop[, list(outcome1=sum(outcome1), 
                               date=min(date)), by=.(intercept, pseudo_nhs_num, year, quarter, dep, cvd, imd, age_2018, lsoa11, pseudo_uprn)]
...
study_pop_ag[date==start , pr_treated:= exp(0.009*age_2018+0.009*imd+0.009*cvd+0.009*dep+0.005*cum_out*0.1+0.09*outcome1-6)
          /(1+
              exp(0.009*age_2018+0.009*imd+0.009*cvd+0.009*dep+0.005*cum_out*0.1+0.09*outcome1-6)
          ), by=.(plac_date)]
}
study_pop_ag[date==plac_date, treated:=rbinom(.N,size=1, prob=pr_treated),by=.(plac_date)]
table(study_pop_ag$treated, useNA = "ifany")
...
study_pop_ag<-study_pop_ag[, .(pseudo_nhs_num, year, quarter, dep,date, date_treated,  cvd, imd, age_2018, lsoa11, pseudo_uprn, outcome1, cum_out, treated,time )]
fwrite(study_pop_ag, "./chess_simulated_data2018.csv")
names(study_pop_ag)
