...
plot1<-ggplot(data=pooled_data, aes(x=date, y=outcome1, group = as.factor(treated), color=as.factor(treated)))+geom_line()+geom_point()+geom_vline(aes(xintercept =date_treated_dt))+facet_wrap(date_treated_dt~.)+theme_minimal() +theme(strip.text.y = element_text(angle = 0, size=7))+scale_color_brewer("",  type = "qual", palette = 2, labels=c("Control", "Intervention"))
plot1
