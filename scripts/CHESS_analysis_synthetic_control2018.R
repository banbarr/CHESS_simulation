...
study_pop_ag<-fread("./data/chess_simulated_data2018_multiple_interventions_vs1.csv")
...
study_pop_ag[lcss_treated==0,lcss_date_treated:=NA ]

source("./scripts/estimate_pooled_did.R")



plot_data<-study_pop_ag[, list(outcome1=mean(outcome1)), by=.(lcss_treated, date, lcss_date_treated, lcss_treated)]
plot1<-ggplot(data=plot_data[year(lcss_date_treated)>2018], aes(x=date, y=outcome1, group = as.factor(lcss_treated),color=as.factor(lcss_treated)))+geom_line(color="purple")+geom_point(color="purple")+geom_vline(aes(xintercept = lcss_date_treated))+facet_wrap( lcss_date_treated~.)+theme_minimal()+theme(strip.text.y = element_text(angle = 0, size=7))
plot1
